...
fun main (args: Array<String>){
                        ...
                        "1" -> {
                            ...
                            if (contadorPlacas == 0){
                                ...
                                listaNiveles.filter{it.id.equals(respuesta3)}.forEach{
                                    it.mapa.forEach{println(it)}
                                }
                                print("Ingrese el parqueo en el que se estacionará: ")
    ...
    var n1 = Nivel("Nivel 1",location,1,"Rojo",listaMapaNull,listaParqueoNull)

    n1.initMapa()

    try {
        File(location).forEachLine { println(it) }
    } catch (e: ExceptionInInitializerError) {}

    for(i in n1.mapa.indices){
        var linea:List<String> = n1.mapa.get(i).split("")
        var limite = linea.size
        for(a in 0..limite-1){
            if(linea.get(a) in elementosEspeciales){
                if(n1.parqueos.filter{it.id == linea.get(a)}.size==0){
                    n1.parqueos.add(Parqueo(a,i,false,linea.get(a)))
                }else{
    ...
    n1.parqueos.forEach{println(it.id)}**/
